package com.android.taco.data

import android.content.SharedPreferences
import com.android.taco.Application
...
class SharedPref{
    companion object {
        ...
        operator fun invoke() : SharedPref = instance ?: synchronized(lock) {
            instance ?: makeCustomSharedPreferences().also {
                instance = it
            }
        }

        private fun makeCustomSharedPreferences() : SharedPref {
            sharedPreferences = androidx.preference.PreferenceManager.getDefaultSharedPreferences(Application.appContext)
            editor = androidx.preference.PreferenceManager.getDefaultSharedPreferences(Application.appContext).edit()
            return SharedPref()
        }
    }
    ...
    fun getSkipOnboarding() : Boolean{
        return sharedPreferences.getBoolean(keySkipOnboarding, false)
    }
